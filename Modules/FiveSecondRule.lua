...
function module:OnLoad()
	...
	local function hasFastRegenBuff()
		local isDrinking = Addon:PlayerHasBuff(430)
		local hasInnervate = Addon:PlayerHasBuff(29166)
		local hasEvocation = Addon:PlayerHasBuff(12051)
	...
	local function greaterBowRegen()
		...
		local found, rank = Addon:PlayerHasBuff(SPELL_ID)
	...
	local function rendRegen()
		...
		local found = Addon:PlayerHasBuff(SPELL_ID)
		if found then
			return regen
		end
	...
	local function updateHandler(self)
		...
		local progress = Addon:Clamp((duration - timeLeft) / duration, 0, 1)
